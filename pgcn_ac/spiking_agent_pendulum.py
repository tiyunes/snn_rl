...
class ActorCritic():
    def __init__(self, order, epsilon, step_size, sigma=0.1, num_states=3, radial_sigma=None):
        self.num_states = num_states
        self.epsilon = epsilon
        self.alpha = step_size
        self.sigma = sigma
        #self.cartpole = CartPole()
        self.pendulum = gym.make("Pendulum-v1", render_mode="human")
        # self.pendulum._max_episode_steps=1600
        self.order = order
        self.lda = 0.5
        # self.w = {}
    ...
    def fourier_feature_state(self, state, method='fourier'):
        ...
        state_norm[0] = (state[0]+self.cos_lim[1]) / \
            (self.cos_lim[1]-self.cos_lim[0])
        state_norm[1] = (state[1]+self.sin_lim[1]) / \
            (self.sin_lim[1]-self.sin_lim[0])
        state_norm[2] = (state[2]+self.v_lim[1]) / \
            (self.v_lim[1]-self.v_lim[0])
    ...
    def e_greedy_action(self, action_ind):
        ...
        prob[action_ind] = (1 - self.epsilon) + (self.epsilon/9)
        e_action = np.random.choice(9, 1, p=prob)
        return (e_action - 4) / 2
        ...
        for i in range(num_episodes):
            ...
            while not done:
                ...
                for k in range(len(self.actors)):
                    o_spikes = self.actors[k].forward(state)
                    o_rates.append(o_spikes)
                o_rates = np.array(o_rates)
                action_rates = np.zeros(9)
                for k in range(9):
                    action_rates[k] = sum(
                ...
                new_state, reward, terminated, truncated, _ = self.pendulum.step(
                    action)
                ...
                for k in range(len(self.actors)):
                    self.actors[k].update_weights(delta_t, state, (2*action+4), np.mean(rewards[-10:]))
            ...
            print("Reward after %s episodes: %s" % (i, total_reward))
...
class SpikingActor():
    def __init__(self):
        ...
        self.hidden = 100
        self.outputs = 9
        self.ih_weights = 0.01*np.random.rand(9, self.hidden, self.inputs)
        self.ih_bias = 0.01*np.random.rand(self.hidden)
        self.ho_weights = 0.01*np.random.rand(self.outputs, self.hidden)
        self.ho_bias = 0.01*np.random.rand(self.outputs)
        self.alpha = 0.00005
    ...
    #     maps = list(itertools.combinations(range(int(self.inputs*0.334)), r=5))
    #     state_code = -1*np.ones(self.inputs)
    #     cosb = int(self.inputs*0.334*(state[0] + 1)/2)
    #     sinb = int(self.inputs*0.334*(state[1] + 1)/2)
    #     vb = int(self.inputs*0.334*(state[2] + 8)/16)
    #     state_code[list(maps[cosb])] = 1
    #     state_code[list(np.array((maps[sinb])) + int(self.inputs*0.334))] = 1
    #     state_code[list(np.array((maps[vb])) + int(self.inputs*0.668))] = 1
    #     return state_code
    ...
    def forward(self, state):
        ...
        self.hz = np.zeros((9, self.hidden))
        self.h_spikes = np.ones((9, self.hidden))
        for i in range(9):
    ...
    def update_weights(self, tderror, state, action, mean_reward):
        ...
        if mean_reward > -900 and mean_reward < -500:
            self.alpha = 0.00005
        elif mean_reward > -500:
            self.alpha = 0.0000005
        else:
            self.alpha = 0.0005
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_episodes', dest='num_episodes', default=1050)
    ...
    for i in range(int(args.num_trials)):
        print('Trial:', i)
        td_cp = ActorCritic(order=5, epsilon=epsilon,
                            step_size=step_size, num_states=3)
    ...
    if args.plot:
        ...
        plt.plot(np.convolve(rewards_mean, np.ones(10)/10, 'valid'))
        plt.ylabel('Mean reward')
        plt.xlabel('Number of episodes')
        plt.show()

    f = open('rewards_ac_pendulum.pkl', 'wb')
    ...
    #         new_state, reward, terminated, truncated, _ = td_cp.pendulum.step(
    #             int(action))
    #         done = terminated or truncated
    ...
    #     f = open('test_rewards_ac_pendulum.pkl', 'wb')
