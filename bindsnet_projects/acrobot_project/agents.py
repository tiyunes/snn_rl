...
class AcrobotObserverAgent(ObserverAgent):
    """
    A agent in Gym Acrobot-v1 environment.
    Parameters
    ----------
    environment : GymEnvironment
        The environment of the observer agent.
    ...
    def build_network(self) -> None:
        ...
        """
        # Input layer
        s2 = Input(shape=[2, 6], traces=True,
        ...
        pm = DiehlAndCookNodes(shape=[3, 1], traces=True,
                               traces_additive=True,
                               tc_trace=1.0,
                               trace_scale=0.0,
                               thresh=-64.0,
        ...
        # Initial weights
        w1 = torch.normal(0.5*torch.ones(s2.n, pm.n), 0.01 * torch.ones(s2.n, pm.n))
        ...
        self.network.add_monitor(
            Monitor(self.network.layers["PM"], ["s"]), "PMS",
        )
        self.network.add_monitor(
            Monitor(self.network.layers["PM"], ["v"]), "PMV",
        )
        self.network.add_monitor(
...
class AcrobotAgent(ObserverAgent):
    """
    A agent in Gym Acrobot-v1 environment.
    Parameters
    ----------
    environment : GymEnvironment
        The environment of the acrobot agent.
    method : str, optional
        The method that the agent acts upon. Possible values are:
            `first_spike`: Select the action with the first spike.
            `softmax`: Select an action using softmax function on the spikes.
    ...
    def build_network(self) -> None:
        ...
        # Add monitors
        self.network.add_monitor(
            Monitor(self.network.layers["PM"], ["s"]), "PM",
        )
        self.network.add_monitor(
            Monitor(self.network.layers["PM"], ["v"]), "PM",
        )
        self.network.add_monitor(
            Monitor(self.network.layers["S2"], ["s"]), "S2"
        )
...
class ExpertAcrobotAgent(Agent):
    """
    Expert agent in Gym environment.
