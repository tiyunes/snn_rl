...
class AcrobotPipeline(EnvironmentPipeline):
    ...
    def step_(
        ...
        # print("log_path: ", self._log_path, ", learning: ", self.observer_agent.network.learning)
        if self._log_path is not None and not self.observer_agent.network.learning:
            self._log_info(self._log_path, obs.squeeze(), inputs)
    ...
    def _train(self, training_act=False, **kwargs):
        while self.episode < self.num_episodes:
            ...
            while not prev_done:
                ...
                self.step_((prev_obs, prev_reward, prev_done, info), **kwargs)
                if self.log_writer:
    ...
    def _save_simulation_info(self, **kwargs):
        ...
        #     self.network.monitors["PMS"].get("s").squeeze(),
        # ], dim=1).nonzero()
        
        spikes = self.network.monitors["PMS"].get("s").squeeze().nonzero()
        
        
        if self.observer_agent.network.learning:
            with open(f"{self._log_path}/train_spikes{self.episode}.txt", 'a') as tr:
                tr.write(f"{self.time_recorder} {spikes}\n")
