...
class PlaceCell(nengo.neurons.NeuronType):
    ...
    def __init__(self, rho_rc=4e7, sigma1=np.pi/3, sigma2=np.pi/3,
...
class SRM0(nengo.neurons.NeuronType):
    ...
    def __init__(self, rho_0=6e1, chi=-5e-3, tau_m=20e-3, threshold=16e-3,
    ...
    def step(self, dt, J, output, voltage, refractory_time):
        ...
        if spikes.any():
            ...
            master.critic_lsindeces[spikes] = (master.critic_lsindeces[spikes] + 1) % time_window_critic
            master.v_plus_trace[spikes] += kappa_plus_filter(sim.time)
            master.v_minus_trace[spikes] += kappa_minus_filter(sim.time)
            
            master.delta_plus_trace[spikes] += kappa_deriv_plus_filter(sim.time)
            master.delta_minus_trace[spikes] += kappa_deriv_minus_filter(sim.time)
...
class Master():
    def __init__(self,
        ...
        self.actors = [SpikingActor() for i in range(5)]
        for i in range(5):
            with open(f'actor_weights/ih_weights_ac_{i}.pkl', 'rb') as ih_weights1:
                self.actors[i].ih_weights = pickle.load(ih_weights1)
            with open(f'actor_weights/ho_weights_ac_{i}.pkl', 'rb') as ho_weights1:
                self.actors[i].ho_weights = pickle.load(ho_weights1)
            # self.actors[i].ih_weights = np.load(f'actor_weights/ih_weights_ac_{i}.pkl')
            # self.actors[i].ho_weights = np.load(f'actor_weights/ho_weights_ac_{i}.pkl')
        ...
        self.input_critic_spike_times = np.zeros((n_critic, n_place, time_window))
        ...
        self.critic_spike_times = np.zeros((n_critic, time_window_critic))
...
#         self.env.render()
    ...
    def calc_td_error(self, t, critic_rates):
        try:
            y_kappa_deriv = np.exp(-sim.time/v_k) * self.delta_plus_trace - np.exp(-sim.time/tau_k) * self.delta_minus_trace
            ...
            y_kappa_deriv = np.exp(-t/v_k) * self.delta_plus_trace - np.exp(-t/tau_k) * self.delta_minus_trace
    ...
    def critic_main(self, t, x):
        ...
        outer = X_conv_eps.reshape(n_critic, n_place, 1) * self.critic_spike_times.reshape(n_critic, 1, time_window_critic)
    ...
    def calc_value(self, t, critic_rates):
        try:
            rho_kappa = np.exp(-sim.time/tau_k) * self.v_plus_trace - np.exp(-sim.time/v_k) * self.v_minus_trace
            # rho_kappa = np.where(self.critic_spike_times != 0, kappa_filter(self.critic_spike_times - sim.time), self.critic_spike_times)
        except:
            rho_kappa = np.exp(-t/tau_k) * self.v_plus_trace - np.exp(-t/v_k) * self.v_minus_trace
        ...
        value = self.v * np.mean(rho_kappa) + self.V_0
        return self.v * np.mean(rho_kappa) + self.V_0
...
with model:
    ...
    # critic_main = nengo.Node(master.critic_main,
